...
public class BoardTUI extends TUI {
	...
	public void showCurrentPlayer() {
		if (game.getCurrentPlayer() instanceof ComputerPlayer) {
			System.out.println("De huidige speler is " + 
	...
	/**
	 * Creeert een vergrotend bord. Kijkt hiervoor naar 
	 * de waarden die in Board.java worden berekend.
	 */
	public void renderBoard() {
		...
		for (int y = game.getBoard().viewOfMaxY; y >= game.getBoard().viewOfMinY; y--) {
			for (int x = game.getBoard().viewOfMinX; x <= game.getBoard().viewOfMaxX; x++) {
				if (game.getBoard().getField(x, y).getColor() == "empty" && 
									game.getBoard().getField(x, y).getSymbol() == "empty") {
					...
					System.out.print(colorRepresentive(game.getBoard().getField(x, y).getColor()) + 
										"-" + symbolRepresentive(
												game.getBoard().getField(x, y).getSymbol()) + "	");
	...
	public void showTiles() {
		try {
			...
			for (int i = 1; i <= tiles.size(); i++) {
				System.out.print(i + ": " + 
								tiles.get(i - 1).getColor() + tiles.get(i - 1).getSymbol() + "  ");
			}
			System.out.println();
